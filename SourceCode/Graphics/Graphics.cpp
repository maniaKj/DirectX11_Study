...
void Graphics::RenderFrame()
{
	...
	{//pavement Texture
		...
		DirectX::XMMATRIX world = XMMatrixScaling(5.0f, 5.0f, 5.0f) * XMMatrixTranslation(translationOffset[0], translationOffset[1], translationOffset[2]); //identifying matrix
		cb_vs_vertexshader.data.mat = world * camera.GetViewMatrix() * camera.GetProjectionMatrix(); //전역 좌표계 * 뷰 행렬 * 투영 행렬
		cb_vs_vertexshader.data.mat = DirectX::XMMatrixTranspose(cb_vs_vertexshader.data.mat); // 다이렉트 x와 hlsl이 행렬을 다루는 방식이 달라서 변환 적용(hlsl-Column major)
		...
		this->deviceContext->RSSetState(this->rasterizerState_CullFront.Get()); //뒤를 먼저 그리고
		this->deviceContext->DrawIndexed(indicesBuffer.BufferSize(), 0, 0);
		this->deviceContext->RSSetState(this->rasterizerState.Get()); //앞을 그리는 방식으로 블레딩 문제 해결-> 나중에 복잡한 물체에 대해서는 해결 불가
		this->deviceContext->DrawIndexed(indicesBuffer.BufferSize(), 0, 0);
	}
	...
	if (ImGui::Button("CLICK ME")) { //button 기능
		counter += 1;
	}
	ImGui::SameLine(); //기본적으로는 명령어마다 ui 줄 바꿈 적용. same line 으로 줄 바꿈 없애기
	std::string clickCount = "Click Cout : " + std::to_string(counter);
	ImGui::Text(clickCount.c_str());
	ImGui::End();
	
	//Window2
	ImGui::Begin("sample02");
	ImGui::DragFloat("Alhpa", &alpha, 0.02f, 0.00f, 1.00f);
	ImGui::End();
...
{
	try {
		//Triangle 버텍스 -> 시계방향으로 설정해야 함(기본 환경)
		Vertex v[] = {
		...
		COM_ERROR_IF_FAILED(hr, "Failed tot initialize vertex buffer.");
		...
		COM_ERROR_IF_FAILED(hr, "Failed tot initialize index buffer.");
